    ...
    <el-upload :class="b({'list':listType=='picture-img'})"
      ...
      <template v-else>
        <el-button size="small"
                   type="primary">{{$lang('点击上传')}}</el-button>
      </template>
      <div slot="tip"
...
export default create({
  ...
  methods: {
    ...
    handleError (msg) {
      ...
      this.$message.error(msg || this.$lang('上传失败'));
    },
    delete (file) {
      if (this.isArray || this.isString) {
        this.text.forEach((ele, index) => {
    ...
    httpRequest (config) {
      ...
      if (!this.validatenull(acceptList) && !acceptList.includes(accept)) {
        this.hide(this.$lang('文件类型不符合'));
        return;
      }
      if (!this.validatenull(filesize) && filesize > this.filesize) {
        this.hide(this.$lang('文件太大不符合'));
        return;
      }
    ...
    beforeRemove (file) {
      return this.$confirm(`${this.$lang('是否确定移除该选项？')}`);
    }
  }
});
</script>
